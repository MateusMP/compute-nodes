...
export default class BaseNode extends React.Component<BaseNodeProps, any> {
    ...
    registerForDragging(element: SVGForeignObjectElement) {
        ...
        const dragCall = d3.drag<SVGForeignObjectElement, any>()
            .filter(() => {
    ...
    render() {
        ...
        return (
            <foreignObject x={x} y={y} width={this.minWidth} height={5} className="embedded-node">
                <div ref={this.dragArea} title={this.props.title} className={`node ${hasError ? 'error' : ''}`}>
                    <Container className="main-node node-drag">
                        <div className={"node-remove"}><span role="img" aria-label="delete">🗑️</span></div>
