...
beforeEach(() => { resolver = new TestNodeResolver(nodeRegistry) });
...
it("sum node resolves", () => {
    let n1: any;
    let n2: any;
    act(() => { n1 = resolver!.createNode('In1Out1', {}); });
    act(() => { n2 = resolver!.createNode('In1Out1', {}); });
    expect(n1.id in resolver!.nodes).toBe(true);
    expect(n2.id in resolver!.nodes).toBe(true);
    expect(resolver!.createPinConnection(buildPinId(n1.id, 'out'), buildPinId(n2.id, 'out')) != false);
    expect(resolver!.createPinConnection(buildPinId(n1.id, 'out'), buildPinId(n1.id, 'out')) == false);
});